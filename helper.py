...
# School Inference
# -----------------------------
def infer_school_context(resume_text, keywords):
    try:
        vector_db = get_vector_main()
        graph_db = get_graph_main()

        resume_docs = vector_db.similarity_search(st.session_state.resume_data, k=SCHOOL_COURSE_INFERENCE_K, filter={"type": "Course"})
